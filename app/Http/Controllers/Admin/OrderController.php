...
class OrderController extends BaseResourceController
{
  ...
  private function submitOrder($id, Request $request)
  {
    $request->validate([
      'message' => 'required',
      'attachment_link' => 'nullable',
    ...
    try {
      ...
      $workHistoryCheck = WorkHistory::where('order_id', $id)->latest()->first();
      ...
      $workHistory = WorkHistory::create($historyData);

      $attachmentData = [
        "work_history_id" => $workHistory->id,
      ];

      $this->saveAttachments($request, Attachment::class, AttachmentType::WorkHistory, $attachmentData);

      $attachmentLinkData = [
        ...$attachmentData,
        'type' => AttachmentType::WorkHistory,
      ...
      if (!empty($workHistoryCheck)) {
        $clientBody = '';
        if ($workHistoryCheck->type === HistoryType::Request && $workHistoryCheck->status === WorkStatusType::Revision) {
          $clientBody = str_replace('{order_id}', $order->order_number, NotifConstants::$CLIENT['SUBMIT_REVISION']);
        } else {
          $clientBody = str_replace('{order_id}', $order->order_number, NotifConstants::$CLIENT['WORK_SUBMITTED']);
        }
        $notifData = [
