...
const DeleteModal = (state) => {
  const { data: product, btnLabel = null, btnClassName = null, handleClick } = state
  const MODAL_TYPE = `delete_${product?.id}`
  const backButtonRef = useRef()
  const { delete: destroy, processing } = useForm()

  const handleSubmit = (e) => {
    e.preventDefault()
    destroy(`/admin/product/${product.id}`, {
      onSuccess: () => {
  ...
  return (
    <WithModal type={MODAL_TYPE} backdrop={true} onClick={handleClick} btnLabel={btnLabel} btnClassName={btnClassName ?? 'btn btn-xs btn-ghost text-white'}>
      <form action=''>
        ...
        <p>Semua transaksi yang terjadi pada produk ini juga akan terhapus</p>
