...
const QuizDeleteModal = (state) => {
  const { id, data = null, content = null, label, isForceDelete = true, handleClick } = state
  const MODAL_TYPE = `quiz_delete_${id}`
  const { page, location, lesson, module} = usePage().props
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    destroy(`${page.url}/${module?.id}/lessons/${lesson?.id}/quizzes/${id}`, {
      onSuccess: () => {
